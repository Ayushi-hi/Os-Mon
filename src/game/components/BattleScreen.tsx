...
function HpBar({ cur, max, name, lv, status, side, xp, xpToNext }:
  { cur:number; max:number; name:string; lv:number; status:string; side:'player'|'wild'; xp?:number; xpToNext?:number }) {
  const pct = Math.max(0, Math.min(100, (cur / max) * 100))
  const col = pct > 50 ? '#3db83d' : pct > 20 ? '#d4a017' : '#c82020'
  const isPlayer = side === 'player'
  const xpPct = (xp != null && xpToNext) ? Math.min(100, (xp / xpToNext) * 100) : 0
  return (
    <div style={{
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:4 }}>
        ...
        <span className="font-pixel" style={{ fontSize:7, color:'#e8a020' }}>Lv.{lv}</span>
      </div>
      {status !== 'none' && (
      ...
      {isPlayer && xp != null && xpToNext != null && (
        <div style={{ marginTop:4 }}>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:2 }}>
            <span className="font-pixel" style={{ fontSize:6, color:'#2244aa' }}>XP</span>
            <span className="font-pixel" style={{ fontSize:6, color:'#2244aa' }}>{xp}/{xpToNext}</span>
          </div>
          <div style={{ height:3, background:'#111', borderRadius:2, overflow:'hidden' }}>
            <div style={{ height:'100%', width:`${xpPct}%`, background:'#3366ff', borderRadius:2, transition:'width 0.5s' }} />
          </div>
        </div>
      )}
    </div>
...
export default function BattleScreen() {
  ...
  useEffect(() => {
    if (battlePhase !== 'menu') { setShowMoves(false); setShowSwitch(false) }
  }, [battlePhase])

  // When battle ends with a loss, show game over then return to title
  useEffect(() => {
    if (battlePhase !== 'end') return
    if (battleResult === 'lose') {
      // Show game over after a short delay
      const t = setTimeout(() => dispatch({ type: 'SET_PHASE', phase: 'gameover' }), 2600)
      return () => clearTimeout(t)
    }
    // Win/caught/ran: return to exploring
    const t = setTimeout(() => dispatch({ type: 'SET_PHASE', phase: 'exploring' }), 2600)
    return () => clearTimeout(t)
  }, [battlePhase, battleResult, dispatch])
  ...
  const resultMsg =
    ...
    battleResult==='lose'   ? `All OS-Mons fainted...`            : ''

  const resultCol =
    battleResult==='win'||battleResult==='caught' ? '#44ff88' :
    battleResult==='ran' ? '#aaaaff' : '#ff6666'

  // Bench: party members that are not the active one
  const bench = party.slice(1)
  ...
  return (
    <div className="absolute inset-0 z-20 flex flex-col"
      ...
      <div className="relative" style={{ flex:'1 1 0', overflow:'hidden' }}>
        ...
        {/* Wild mon — top right */}
        <div style={{ position:'absolute', top:16, right:80, filter: isEnd && battleResult==='win' ? 'grayscale(1) opacity(0.3)' : undefined, transition:'filter 0.5s' }}>
        ...
        <div style={{ position:'absolute', bottom:72, left:40, filter: isEnd && battleResult==='lose' ? 'grayscale(1) opacity(0.3)' : undefined, transition:'filter 0.5s' }}>
          <MonSprite mon={player} size={118} flip={false} animate={!isEnd} />
        </div>
        <div style={{ position:'absolute', bottom:68, right:14 }}>
          <HpBar cur={player.curHp} max={player.maxHp} name={player.name} lv={player.lv}
            status={player.status} side="player"
            xp={player.xp ?? 0} xpToNext={player.xpToNext} />
          <div className="font-pixel" style={{ fontSize:6, color:'#2a3a5a', padding:'2px 14px',
            background:'rgba(4,4,20,0.70)', borderTop:'1px solid #1a3a6e' }}>
            ATK:{player.atkScaled ?? player.attack} &nbsp; DEF:{player.defScaled ?? player.defense} &nbsp; SPD:{player.spdScaled ?? player.speed}
          </div>
        </div>
        ...
        {isEnd && resultMsg && (
          <div className="absolute inset-0 flex items-center justify-center"
            style={{ background:'rgba(0,0,0,0.65)', animation:'fadeIn 0.3s ease' }}>
            <div className="font-pixel text-center" style={{
              ...
              <div className="font-vt" style={{ fontSize:16, color:'#556', marginTop:6 }}>
                {battleResult === 'lose' ? 'Blacking out...' : 'Returning to world...'}
              </div>
            </div>
          </div>
        )}
          ...
          {battleLog.slice(-5).map((line, i) => (
            <div key={i} style={{
              color: line.includes('fainted') ? '#ff8888' :
                     ...
                     line.includes('appeared') || line.includes('battle') ? '#ffffaa' :
                     line.includes('used') ? '#aaccff' :
                     line.includes('Go,') ? '#44ffaa' : '#aab',
              opacity: i < battleLog.slice(-5).length - 1 ? 0.6 + i * 0.1 : 1,
            }}>{line}</div>
          ))}
        </div>

        {/* Action panel */}
        <div style={{ width:380, padding:'10px 12px', display:'flex', flexDirection:'column', gap:8 }}>
          {isEnd ? (
            ...
            // ── SWITCH PANEL ──
            <>
              <div className="font-pixel" style={{ fontSize:7, color:'#e8a020', marginBottom:2 }}>
                ⇄ SWITCH OS-MON
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:5, overflowY:'auto', maxHeight:150 }}>
                {party.map((mon, i) => {
                  ...
                  const pct = Math.max(0, Math.min(100, (mon.curHp / mon.maxHp) * 100))
                  const hpCol = pct > 50 ? '#3db83d' : pct > 20 ? '#d4a017' : '#c82020'
                  return (
                    <button
                      key={i}
                      disabled={isCurrent || fainted || isAnimating}
                      className="font-pixel cursor-pointer text-left"
                      style={{
                        padding:'7px 10px', fontSize:7,
                        background: isCurrent ? '#0a1020' : fainted ? '#150505' : '#080818',
                        border: isCurrent ? '1.5px solid #2255aa' : fainted ? '1.5px solid #441' : '1.5px solid #1a3a6e',
                        color: fainted ? '#554' : isCurrent ? '#88aaff' : '#ccd',
                        opacity: fainted ? 0.5 : 1,
                        transition:'all 0.1s',
                      }}
                      onClick={() => {
                        dispatch({ type:'SWITCH_OSMON', partyIndex: i })
                        setShowSwitch(false)
                      }}>
                      <div style={{ display:'flex', justifyContent:'space-between' }}>
                        <span>{mon.name}{isCurrent ? ' (active)' : fainted ? ' ✕' : ''}</span>
                        <span style={{ color:'#e8a020' }}>Lv.{mon.lv}</span>
                      </div>
                      <div style={{ marginTop:3, height:4, background:'#111', borderRadius:2, overflow:'hidden' }}>
                        <div style={{ height:'100%', width:`${pct}%`, background: hpCol, borderRadius:2 }} />
                      </div>
                      <div style={{ fontSize:6, color:'#556', marginTop:2 }}>{mon.curHp}/{mon.maxHp} HP</div>
                    </button>
                  )
                })}
              </div>
              <button className="font-pixel cursor-pointer"
                ...
                onClick={() => setShowSwitch(false)}>← BACK</button>
            </>
          ) : !showMoves ? (
            // ── MAIN MENU ──
            <>
              <div style={{ display:'flex', gap:8 }}>
                <button className="font-pixel cursor-pointer" disabled={isAnimating}
                  ...
                  onClick={() => setShowMoves(true)}>⚔ FIGHT</button>
                <button className="font-pixel cursor-pointer" disabled={isAnimating || items.kernelball <= 0}
                  ...
                  onClick={() => dispatch({ type:'THROW_BALL', ballType:'kernelball' })}>
                  ◉ BALL <span style={{ fontSize:7, color:'#4488ff' }}>({items.kernelball})</span>
                </button>
              </div>
              <div style={{ display:'flex', gap:8 }}>
                <button className="font-pixel cursor-pointer" disabled={isAnimating || bench.length === 0}
                  style={{ flex:1, padding:'10px 6px', fontSize:8, background:'#081828', color:'#88aaff', border:'1.5px solid #2255aa', transition:'all 0.12s',
                    opacity: bench.length === 0 ? 0.4 : 1 }}
                  onMouseEnter={e=>{ if(bench.length>0)(e.currentTarget as HTMLButtonElement).style.background='#0a1e30'}}
                  onMouseLeave={e=>{ (e.currentTarget as HTMLButtonElement).style.background='#081828'}}
                  onClick={() => setShowSwitch(true)}>⇄ SWITCH</button>
              ...
              <div style={{ display:'flex', gap:8 }}>
                <button className="font-pixel cursor-pointer" disabled={isAnimating}
                  style={{ flex:1, padding:'8px 6px', fontSize:7, background:'#081808', color:'#88ff88', border:'1px solid #44aa44', transition:'all 0.12s' }}
                  ...
                  onClick={() => dispatch({ type:'RUN' })}>↩ RUN</button>
                {items.superball > 0 && (
                  <button className="font-pixel cursor-pointer" disabled={isAnimating}
                    style={{ flex:1, padding:'8px 6px', fontSize:7, background:'#0a1030', color:'#aaddff', border:'1px solid #2255aa' }}
                    onClick={() => dispatch({ type:'THROW_BALL', ballType:'superball' })}>
                    ◎ S.BALL ({items.superball})
                  </button>
                )}
              </div>
            </>
            ...
            <>
              <div style={{ display:'flex', gap:6 }}>
                {player.moves.slice(0,2).map(mId => (
                  <MoveBtn key={mId} moveId={mId} disabled={isAnimating}
                    onClick={() => { dispatch({type:'PLAYER_MOVE',moveId:mId}); setShowMoves(false) }} />
                ))}
              </div>
              <div style={{ display:'flex', gap:6 }}>
              ...
              <button className="font-pixel cursor-pointer"
                style={{ padding:'7px', fontSize:7, background:'#0a0a14', color:'#445', border:'1px solid #1a2a3a' }}
                onClick={() => setShowMoves(false)}>← BACK</button>
            </>
          )}
        </div>
      </div>
